...
# Adjusted thresholds for detecting slight/extreme movement
SLIGHT_THRESHOLD= 0.03
EXTREME_THRESHOLD= 0.08  # Adjusted for better separation between slight and extreme
IRIS_THRESHOLD = 0.015
...
#     return ratio
def calculate_face_direction(landmarks, img_w, img_h):
    nose = landmarks[NOSE_TIP_INDEX]
    chin = landmarks[CHIN_INDEX]
    left_eye = landmarks[LEFT_EYE_TOP]
    right_eye = landmarks[RIGHT_EYE_TOP]
    left_eye_left = landmarks[LEFT_EYE_LEFT_CORNER_INDEX]
    right_eye_right = landmarks[RIGHT_EYE_RIGHT_CORNER_INDEX]
    ...
    eye_center_x = (left_eye_left[0] + right_eye_right[0]) / 2
    rel_x = (nose[0] - eye_center_x) / face_width

    h_direction = "Center"
    if abs(rel_x) > SLIGHT_THRESHOLD:
        h_direction = "Extreme " if abs(rel_x) > EXTREME_THRESHOLD else "Slight "
        h_direction += "Left" if rel_x < 0 else "Right"

    # Vertical direction calculation
    eye_center = (left_eye + right_eye) / 2
    eye_nose_dist = nose[1] - eye_center[1]
    eye_chin_dist = chin[1] - eye_center[1]
    
    # Calculate the ratio of eye-nose distance to eye-chin distance
    vertical_ratio = eye_nose_dist / eye_chin_dist

    # Define thresholds for vertical direction
    up_threshold = 0.30  # Fine-tuned threshold for upward direction
    down_threshold = 0.40  # Refined downward threshold

    if vertical_ratio < up_threshold:
        v_direction = "Extreme Up" if vertical_ratio < up_threshold / 2 else "Slight Up"
    elif vertical_ratio > down_threshold:
        # More granular distinction for extreme downward tilts
        extreme_down_threshold = down_threshold + 0.05  # Adjust for finer distinction
        v_direction = "Extreme Down" if vertical_ratio > extreme_down_threshold else "Slight Down"
    else:
        v_direction = "Center"

    return h_direction, v_direction, rel_x, vertical_ratio

def detect_iris_direction(left_iris_center, right_iris_center, left_eye, right_eye):
    left_eye_center = (left_eye[0] + left_eye[1]) / 2
    right_eye_center = (right_eye[0] + right_eye[1]) / 2
    
    left_eye_width = np.linalg.norm(left_eye[1] - left_eye[0])
    right_eye_width = np.linalg.norm(right_eye[1] - right_eye[0])
    
    left_rel_x = (left_iris_center[0] - left_eye_center[0]) / (left_eye_width / 2)
    right_rel_x = (right_iris_center[0] - right_eye_center[0]) / (right_eye_width / 2)
    
    avg_rel_x = (left_rel_x + right_rel_x) / 2
    
    h_direction = "Center"
    if abs(avg_rel_x) > IRIS_THRESHOLD:
        h_direction = "Left" if avg_rel_x < 0 else "Right"
    
    return h_direction, avg_rel_x

def detect_eye_direction(face_h_direction, face_v_direction, iris_h_direction):
    eye_direction = {"horizontal": "Center", "vertical": face_v_direction}
    
    if face_h_direction == "Center":
        eye_direction["horizontal"] = iris_h_direction
    else:
        eye_direction["horizontal"] = face_h_direction.replace("Slight ", "").replace("Extreme ", "")
...
def detect_iris(landmarks, img):
    def draw_iris(iris_landmarks, color):
        iris_points = np.array([landmarks[idx][:2] for idx in iris_landmarks])
        (x, y), radius = cv.minEnclosingCircle(iris_points)
        center = np.array([x, y], dtype=np.int32)
        cv.circle(img, center, int(radius), color, 1, cv.LINE_AA)
        return center, int(radius)

    left_iris_center, left_iris_radius = draw_iris(LEFT_EYE_IRIS, (0, 255, 0))  # Green for left iris
    right_iris_center, right_iris_radius = draw_iris(RIGHT_EYE_IRIS, (0, 255,0))  # Red for right iris

    return left_iris_center, right_iris_center, left_iris_radius, right_iris_radius

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Advanced Eye Direction Detection")
    parser.add_argument("-i", "--imagePath", help="Path to the image file", required=True)
...
#         print(f"Head pose estimation is {head_pose_status}.")
    ...
    cam_source = args.imagePath
    frame = cv.imread(cam_source)
    if frame is None:
        raise ValueError(f"Image at path {cam_source} could not be loaded.")
    ...
    if results.multi_face_landmarks:
        ...
        face_h_direction, face_v_direction, rel_x, nose_to_mouth_dist = calculate_face_direction(mesh_points, img_w, img_h)

        left_iris_center, right_iris_center, left_iris_radius, right_iris_radius = detect_iris(mesh_points, frame)

        iris_h_direction, avg_rel_x = detect_iris_direction(
            left_iris_center, right_iris_center, 
            [mesh_points[LEFT_EYE_LEFT_CORNER_INDEX], mesh_points[LEFT_EYE_RIGHT]],
            [mesh_points[RIGHT_EYE_LEFT], mesh_points[RIGHT_EYE_RIGHT_CORNER_INDEX]]
        )

        eye_direction = detect_eye_direction(face_h_direction, face_v_direction, iris_h_direction)

        print(f"Face Horizontal Direction: {face_h_direction}")
        print(f"Face Vertical Direction: {face_v_direction}")
        # print(f"Iris Horizontal Direction: {iris_h_direction}")
        print(f"Eye Horizontal Direction: {eye_direction['horizontal']}")
        print(f"Eye Vertical Direction: {eye_direction['vertical']}")
       

        cv.putText(frame, f"Face H: {face_h_direction}", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv.putText(frame, f"Face V: {face_v_direction}", (10, 60), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv.putText(frame, f"Eye H: {eye_direction['horizontal']}", (10, 90), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv.putText(frame, f"Eye V: {eye_direction['vertical']}", (10, 120), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

        # cv.circle(frame, tuple(left_iris_center), left_iris_radius, (0, 255, 0), 2)
        # cv.circle(frame, tuple(right_iris_center), right_iris_radius, (0, 255, 255), 2)

        cv.imshow("Advanced Eye Direction Detection", frame)
        cv.waitKey(0)
